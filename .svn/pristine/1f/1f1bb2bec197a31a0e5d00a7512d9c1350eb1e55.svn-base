 ...
 *
 * @cdk.module structgen @cdk.githash
 */
public class LocalCrossoverMachine {
    ...
    /**
     * Indicates that
     * <code>crossover</code> is using SPLIT_MODE_RADNDOM mode.
     */
    public static final int SPLIT_MODE_RADNDOM = 0;
    /**
     * Indicates that
     * <code>crossover</code> is using SPLIT_MODE_DEPTH_FIRST mode.
     */
    public static final int SPLIT_MODE_DEPTH_FIRST = 1;
    /**
     * Indicates that
     * <code>crossover</code> is using SPLIT_MODE_BREADTH_FIRST mode.
     */
     ...
     */
    public LocalCrossoverMachine() {
        pfsm = new PartialFilledStructureMerger();
    }

    /**
     * Performs the n point crossover of two {@link IAtomContainer}. Precondition: The atoms in
     * the molecules are ordered by properties to preserve (e. g. atom symbol). Due to its
     * randomized nature, this method fails in around 3% of all cases. A CDKException with message
     * "Could not mate these properly" will then be thrown.
     *
     * @return The children.
     * @throws CDKException if it was not possible to form offsprings.
     */
