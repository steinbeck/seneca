...
public class StructureMerger {
    ...
    public IAtomContainer generate(IAtomContainerSet atomContainers) throws CDKException {
        ...
        } while (!structureFound && iteration < 50);
        if (atomContainers.getAtomContainerCount() == 1 && satCheck.allSaturated(atomContainers.getAtomContainer(0))) {//&& satCheck.allSaturated(atomContainers.getAtomContainer(0)
            structureFound = true;
        }
        if (!structureFound) {
