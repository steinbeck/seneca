...
/**
 * The InternalVicinitySampler is a generator of Constitutional Isomers. It needs to be provided
 * with a starting constitution and it makes random moves in constitutional space from there. This
 * generator was first suggested by Faulon {@cdk.cite FAU96}.
 *
 * @cdk.keyword structure generator @cdk.module structgen @cdk.githash @cdk.bug 1632610
 */
public class InternalVicinitySampler {

    private final static Logger logger = Logger.getLogger(InternalVicinitySampler.class);
    int molCounter = 0;
                ...
                for (int y1 = x2 + 1; y1 < nrOfAtoms; y1++) {
                    for (int y2 = y1 + 1; y2 < nrOfAtoms; y2++) {
                        ...
                        if (nonZeroBondsCounter > 1) {
                                          /*
                                           * Compute the range for b11 (see Faulons formulae for
                                           * details)
                                           */
                            ...
                            for (b11 = lowerborder; b11 <= upperborder; b11++) {
                                if (b11 != a11) {
                                    ...
                                    try {
                                        newAc = (IAtomContainer) Utilities.clone(ac);
