...
public class RandomGenerator {
    ...
    private IAtomContainer toMutate = null;
...
//                              annealingEvolutionLogger.debug("RandomGenerator->mutate(): x1, x2, y1, y2: " + x1 + ", " + x2 + ", " + y1 + ", " + y2);
//                        } while (!(x1 != x2 && x1 != y1 && x1 != y2 && x2 != y1 && x2 != y2 && y1 != y2));

                x1 = random.nextInt(nrOfAtoms);
                //    System.out.println("Random atom : " + x1);
                // now select 3 atoms in vicinity
                ax1 = ac.getAtom(x1);
                List<Integer> connectedAtoms = getConnectedAtoms(ax1, toMutate.getAtomCount());
                System.out.println("connected atoms: " + connectedAtoms.toString());
                int[] randomAtomIndices = getRandomAtomIndices(connectedAtoms.size());
                if (randomAtomIndices.length == 3) {
                    System.out.println("length 3: chosen atoms: " + connectedAtoms.get(randomAtomIndices[0]) + "," + connectedAtoms.get(randomAtomIndices[1]) + "," + connectedAtoms.get(randomAtomIndices[2]));
                    y1 = connectedAtoms.get(randomAtomIndices[0]);
                    x2 = connectedAtoms.get(randomAtomIndices[1]);
                    y2 = connectedAtoms.get(randomAtomIndices[2]);
                }
...
//                        ax1 = ac.getAtom(x1);
                ...
                // need atleast 2 bonds to change connectivity
            } while (nonZeroBondsCounter < 2);
            //  System.out.println("non zero bonds counter : " + nonZeroBondsCounter);
        ...
        //System.out.println("third loop fine");
    ...
    /**
     * Returns the molecule which reflects the current state of this stochastic structure
     * generator.
     *
     * @return The molecule
     */
    ...
    private List<Integer> getConnectedAtoms(IAtom atom, int totalCount) {
        //System.out.println("getting connected atoms..");
        List<IAtom> initialSphere = new ArrayList<IAtom>();
        List<Integer> tracedAtoms = new ArrayList<Integer>();
        initialSphere.add(atom);
        int percentage = (int) totalCount / 3;
        if (percentage < 3) {
            percentage = 3;
        }
        //System.out.println("Total: " + totalCount + " : percentage: " + percentage);
        removeAllVisitedFlagsOf(toMutate);
        breadthFirstSearch(initialSphere, tracedAtoms, percentage);

        return tracedAtoms;
    }

    private IAtomContainer removeAllVisitedFlagsOf(IAtomContainer molecule) {
    ...
    private void breadthFirstSearch(List<IAtom> sphere, List<Integer> tracedMolecule, int percentage) {
        IAtom nextAtom;
        List<IAtom> newSphere = new ArrayList<IAtom>();
        int spheresLookedUp = 0;
        for (IAtom atom : sphere) {
            spheresLookedUp++;
            int atomNumber = toMutate.getAtomNumber(atom);
            // System.out.println("added: " + atomNumber);
            if (!tracedMolecule.contains(atomNumber)) {
                tracedMolecule.add(atomNumber);
            }
            // now look at bonds
            List<IBond> bonds = toMutate.getConnectedBondsList(atom);
            // System.out.println("connected bonds: " + bonds.size());
            for (IBond bond : bonds) {
                if (!bond.getFlag(CDKConstants.VISITED)) {
                    bond.setFlag(CDKConstants.VISITED, true);
                    //     System.out.println("one bond visited");
                }
                nextAtom = bond.getConnectedAtom(atom);
                if (!nextAtom.getFlag(CDKConstants.VISITED)) {
                    newSphere.add(nextAtom);
                    nextAtom.setFlag(CDKConstants.VISITED, true);
                    //  System.out.println("next atom added");
                }
            }
            if (tracedMolecule.size() > percentage) {
                //System.out.println("BFS: returning... " + tracedMolecule.size());
                return;
            }
        }
        if (newSphere.size() > 0) {
            //System.out.println("searching again for new sphere: " + newSphere.size());
            breadthFirstSearch(newSphere, tracedMolecule, percentage);
        }
    }

    private int[] getRandomAtomIndices(int atomCount) {
        ...
        for (int i = 0; i < result.length; i++) {
            int newRandom;
            do {
                newRandom = random.nextInt(atomCount);
